...
# -----------------------------------------------------------------------
# IMPORTANTE - RENDER / PRODUCCIÓN:
# Antes esta línea SIEMPRE apuntaba a un archivo SQLite local, sin
# importar que en Render se haya conectado una base de datos PostgreSQL.
# Eso era el problema real: Render borra el disco local (donde vivía ese
# archivo .db) cada vez que se hace un nuevo deploy o el servicio se
# reinicia (por ejemplo al "despertar" en el plan gratis), así que la
# app volvía a crear la cuenta del Dueño desde cero con la contraseña
# de fábrica, perdiendo cualquier cambio de contraseña o cuenta nueva
# que hubieras hecho antes de ese reinicio.
#
# Ahora, si existe la variable de entorno DATABASE_URL (Render la crea
# sola en cuanto conectas tu base PostgreSQL a este servicio, dentro de
# "Environment"), se usa esa base de datos real, que SÍ es permanente.
# Si no existe (por ejemplo cuando corres el proyecto en tu compu local),
# se usa el archivo SQLite de siempre, sin romper nada de tu flujo local.
DATABASE_URL = os.environ.get('DATABASE_URL')
USING_POSTGRES = bool(DATABASE_URL)

if DATABASE_URL:
    # Render (igual que Heroku) entrega la URL con el prefijo "postgres://",
    # pero las versiones recientes de SQLAlchemy exigen "postgresql://".
...
# En Render (sin disco persistente) el archivo secret.key también se
# borraría en cada reinicio, lo que invalidaría las sesiones de todos los
# que tuvieran la sesión abierta (los desconectaría, aunque su usuario y
# contraseña sigan intactos en la base de datos). Por eso, si existe la
# variable de entorno SECRET_KEY (puedes definirla tú mismo en Render, en
# "Environment", con cualquier texto largo y aleatorio), se usa esa y se
# vuelve fija para siempre. Si no existe, se sigue usando el archivo local
# de siempre (perfecto para cuando corres el proyecto en tu computadora).
...
with app.app_context():
    ...
    # Estas migraciones manuales (agregar columnas con ALTER TABLE) solo
    # tienen sentido para una base SQLite vieja que ya traía tablas SIN
    # esas columnas. En PostgreSQL (Render) la base arranca vacía y
    # db.create_all() ya crea las tablas completas, con todas las
    # columnas, tal como están definidas en models.py. Ejecutar código de
    # sqlite3 contra Postgres no funcionaría (son motores distintos), así
    # que este bloque se salta por completo cuando se usa Postgres.
    ...
    if not USING_POSTGRES and not os.path.exists(_schema_v2_marker):
        con = sqlite3.connect(DB_PATH)
        cur = con.cursor()
    ...
    # con este cambio:
    #
    # 1) El DUEÑO (OWNER_SEED_USERNAME) -> puede entrar a "Usuarios" para
    #    crear más cuentas. No trae inventario propio de fábrica: empieza
    #    limpio, como cualquier cuenta nueva.
    #
    ...
    #    con cuentas nuevas. TAMBIÉN se le da rol de Dueño ('owner'), para
   ...
   # -----------------------------------------------------------------
    ...
    # aquí también, para que el cambio aplique sin importar cuándo se
    # haya creado esa cuenta. Es idempotente: si ya es 'owner', no hace
    # nada.
    if legacy_user and legacy_user.role != 'owner':
